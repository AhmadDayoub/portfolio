...
const Modal = ({activeID, setShowModal}) => {
  ...
  return (
   <div className='first w-full h-full md:w-full fixed top-0 left-0 z-10 bg-headingColor  bg-opacity-40 '>
        <div className='w-11/12   md:max-w-[600px] absolute top-1/2 left-1/2 z-20 bg-white rounded-[8px] trasnform -translate-x-1/2 -translate-y-1/2 p-5 '>
                <div>
                    <figure>
        ...
        <div>
            <h2 className='text-2xl text-headingColor font-[700] my-7'>{portfolio.title}</h2>
       ...
       <button onClick={() => setShowModal(false)} className='w-[1.8rem] h-[1.8rem] bg-[white] absolute top-[1.7rem] right-[1.7rem] text-[25px] 
              rounded-[3px] leading-0 cursor-pointer flex justify-center items-center'>&times;</button>
      </div>
